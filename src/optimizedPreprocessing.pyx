...
nertagger = StanfordNERTagger("/Users/yanndubois/NLP/stanford-ner-2016-10-31/classifiers/english.all.3class.distsim.crf.ser.gz",
                                      "/Users/yanndubois/NLP/stanford-ner-2016-10-31/stanford-ner.jar")
postagger = StanfordPOSTagger("/Users/yanndubois/NLP/stanford-postagger-full-2016-10-31/models/english-bidirectional-distsim.tagger",
                                      "/Users/yanndubois/NLP/stanford-postagger-full-2016-10-31/stanford-postagger.jar")

cpdef str spellCheckSuggest(str word):
    if word[0].isupper():
        # if noun just return
        return word
    if d.check(word):
    ...
    cdef str bestWord = word
    cdef float bestRatio = 0
    cdef float currentRatio = 0
    cdef set suggestedWords = set(d.suggest(word))
    cdef str suggestedWord
...
cpdef tuple cleaningPOS(str phrase, bool isReplaceNumber=False):
    ...
    cdef list arrayPhrase = phrase.split()
    phrase=""
    cdef int index
    cdef str word
    ...
    for index, word in enumerate(arrayPhrase):
        if not (d.check(word)):
            arrayPhrase[index] = spellCheckSuggest(word)
    
    
    # Computes NER (POS for nouns): can give Person / Organization / Location
    cdef list pos = [x[1] for x in postagger.tag(arrayPhrase)]
    
    # Computes NER (POS for nouns): can give Person / Organization / Location
    cdef list ner = [x[1] for x in nertagger.tag(arrayPhrase)]
...
def preprocessChunk(chunk):
    ...
    cdef str oldPOS1= ""
    cdef str oldPOS2= ""
    cdef str oldPhrase1final= ""
    cdef str oldPhrase2final= ""
